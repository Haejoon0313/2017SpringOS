...
thread_unblock (struct thread *t) 
{
  ...
  list_insert_ordered (&ready_list, &t->elem,priority_compare_two_thread,0);
...
next_thread_to_run (void) 
{
  if (list_empty (&ready_list))
	...
	{ //before we select from list, sorting the list according to priority of each thread.
		//list_sort(&ready_list,priority_compare_two_thread,NULL);			
